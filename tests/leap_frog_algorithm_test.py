...
from Code.leap_frog_algorithm import update_all_positions, calculate_all_forces, update_all_velocities
...
class TestUpdateAllVelocities(unittest.TestCase):
    obj1 = Massiveobject("obj1", 1, np.array([0,0,0]), np.array([3,0,0]), np.array([10,0,0]))
    obj2 = Massiveobject("obj2", 2, np.array([1,0,0]), np.array([4,0,0]), np.array([30,0,0]))
    obj3 = Massiveobject("obj3", 3, np.array([2,0,0]), np.array([5,0,0]), np.array([30,0,0]))
